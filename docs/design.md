...
本系统实现一个基于 C++17 + Qt6 Widgets 的图形界面五子棋游戏。玩家启动程序后先注册黑方和白方姓名，再通过鼠标点击 15 x 15 棋盘交叉点直接落子，黑白双方轮流下棋。程序需要阻止重复落子，支持悔棋、认输、重新开始和游戏结束后复盘，并在横向、纵向或斜向形成五子连珠时弹窗提示“阵营 + 玩家姓名”。
...
当前 GUI 版本已接入复盘按钮，游戏结束后可以按历史落子逐步查看棋盘状态。
