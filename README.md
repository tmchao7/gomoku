...
这是一个使用 C++17 + Qt6 Widgets 实现的图形界面五子棋项目。项目支持普通模式、进阶模式和人机对战模式：普通模式是传统五连即胜；进阶模式加入"五连消除、得分、转化对方棋子、连锁判定"等规则；人机模式使用启发式 AI 与玩家对弈。
...
- 鼠标点击棋盘交叉点直接落子
- 启动时注册黑方、白方玩家姓名
- 启动时选择游戏模式（玩家对战-普通、玩家对战-进阶、人机模式）
- 人机模式下玩家执黑先手，AI 执白后手
- 已有棋子位置不可重复落子
- 最近一步落子有红色圆点高亮标记
- 支持悔棋、认输、重新开始
...
- 玩家执黑先手，AI 执白后手。
- AI 使用启发式打分策略：对每个空位从进攻（己方连子）和防守（对方连子）两个角度综合评分，选取最优位置。
- AI 会在 300ms 延迟后落子，给玩家留出感知时间。
- 悔棋一次撤销两步（AI 上一步 + 玩家上一步）。
...
- `AI` 负责启发式打分策略，评估每个空位的进攻/防守价值并选择最优落子，不依赖 Qt。
